...
use ElementorForge\MCP\Tools\CreateHeader;
...
final class CreateHeaderTest extends TestCase {
	...
	protected function setUp(): void {
		...
		Functions\when( 'get_permalink' )->justReturn( 'https://example.com/header' );
	...
	public function test_input_schema_requires_preset(): void {
		$schema = CreateHeader::input_schema();
	...
	public function test_input_schema_preset_enum_matches_presets_constant(): void {
		...
		$this->assertSame( HeaderPresets::PRESETS, $schema['properties']['preset']['enum'] );
	}


	public function test_output_schema_has_post_id_preset_url(): void {
		$schema = CreateHeader::output_schema();
	...
	public function test_permission_requires_manage_options(): void {
		$this->assertTrue( CreateHeader::permission() );
	}
	...
	public function test_execute_respects_gate_check_in_read_only_mode(): void {
		...
		$result = CreateHeader::execute( array( 'preset' => 'business' ) );
	...
	public function test_execute_returns_error_for_invalid_preset(): void {
		$result = CreateHeader::execute( array( 'preset' => 'nonexistent_preset' ) );
	...
	public function test_execute_returns_preset_in_result(): void {
		$result = CreateHeader::execute( array( 'preset' => 'saas' ) );

		$this->assertIsArray( $result );
		$this->assertSame( 'saas', $result['preset'] );
	}

	public function test_execute_returns_url_in_result(): void {
		$result = CreateHeader::execute( array( 'preset' => 'blog' ) );

		$this->assertIsArray( $result );
		$this->assertSame( 'https://example.com/header', $result['url'] );
	}
	...
	public function test_execute_returns_error_when_installer_returns_zero(): void {
		...
		$this->assertInstanceOf( WP_Error::class, $result );
		$this->assertSame( 'elementor_forge_header_install_failed', $result->get_error_code() );
	}
	...
	public function test_execute_succeeds_for_all_valid_presets( string $preset ): void {
		$result = CreateHeader::execute( array( 'preset' => $preset ) );
	...
	public function test_execute_forwards_overrides_to_header_presets(): void {
		$result = CreateHeader::execute(
			array(
				'preset'    => 'business',
				'overrides' => array( 'background_color' => '#ff00ff' ),
			)
		);
